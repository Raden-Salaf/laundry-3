...
<aside class="sidebar-wrapper d-flex flex-column text-white shadow">

    {{-- Brand --}}
    <div class="px-4 py-4 d-flex align-items-center gap-3 border-bottom border-white border-opacity-10">
        <div class="rounded-3 bg-primary bg-gradient d-flex align-items-center justify-content-center fs-5 text-white" style="width: 40px; height: 40px;">
            🫧
        </div>
        <div>
            <h6 class="m-0 fw-bold leading-tight text-white">Fresh Laundry</h6>
    ...
    <div class="p-3 border-top border-white border-opacity-10">
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="btn btn-outline-danger w-full d-flex align-items-center justify-content-center gap-2 py-2 text-white border-0 opacity-75 hover-bg-danger rounded-3" style="transition: all 0.2s;">
                <span>🚪</span> Logout
            </button>
        </form>
    </div>
